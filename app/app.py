...
import os
from flask import Flask, flash, request, redirect, url_for, session, send_file, abort, jsonify
...
def upload_file():
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            print(session["uploaded_filepath"])
            ...
            report_json_names = elevadr.run_analysis()
            report = {
                "submitted_pcap": session.get("uploaded_filepath").split("/")[-1].split(".")[0],
...
@app.route('/report/<submission>/<filename>')
def get_file(submission, filename):
    file_path = str(Path(ZEEK_OUTPUT_DIR, submission, filename)) # This could be dangerous if it allows access to other directories. Need to implement input validation
